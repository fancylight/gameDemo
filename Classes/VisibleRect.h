...
public:
	...
	static cocos2d::Rect splitScreenAsRect(int row, cocos2d::Vec2 vec, int weightScaleToPadding = 2,
		int heightScaleToPadding = 3, int column = 10, float witdhAll = cocos2d::Director::getInstance()->getOpenGLView()->getVisibleRect().size.width, float heightAll = cocos2d::Director::getInstance()->getOpenGLView()->getVisibleRect().size.height);	//将屏幕分为vec2(numberX,numberY) numberX*numberY 数量的矩形,具体位置由c(x,y) 决定,左下角为c(0,0),并且要设定横纵间距比例
private:
